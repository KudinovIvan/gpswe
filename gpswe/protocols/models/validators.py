...
class DataFieldNames(Enum):
    """
    Наименование проверяемых полей
    """
...
class CoordSignFieldNames(Enum):
    """
    Значение для расставления знаков для координат
    """
...
def check_datetime(value: str, field_name: DataFieldNames):
    ...
    try:
        if field_name == DataFieldNames.DATE:
            return "1", datetime.datetime.strptime(value, "%d%m%y").date()
        elif field_name == DataFieldNames.TIME:
            return "1", datetime.datetime.strptime(value, "%H%M%S.%f").time()
    ...
    try:
        if field_name == DataFieldNames.LAT:
            if sign == CoordSignFieldNames.N.value:
                return "1", round(int(value[:2]) + float(value[2:]) / 60, 7)
            ...
            if sign == CoordSignFieldNames.E.value:
                return "1", round(int(value[:3]) + float(value[3:]) / 60, 7)
...
def check_speed_course_alt_sats_hdop(value: str, field_name: DataFieldNames = None):
    """
    Проверка скорости, курса, высоты, кол-ва спутников, hdop, inputs и outputs
    """
    try:
        if value == "NA":
            return "1", None
...
def check_params(value):
    """
    Проверка доп. параметров Wialon
    """
    if " " in value:
