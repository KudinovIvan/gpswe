...
async def headers_package(data: bytearray):
    ...
    frame_data_length = c_uint16(data[5] << 8 * 0).value + c_uint16(data[6] << 8 * 1).value
    ...
    return ResultCodes.EGTS_PC_OK.value, pid, {
...
async def data_encode_package(data: bytearray, current_offset: int, header_length: int, frame_data_length: int):
    encoded_data = []
        ...
        if oid:
            while srd_offset < record_len:
                ...
                if subrecord_type == EGTS_SR_RECORD_RESPONSE:
                    print("parsing EGTS_SR_RECORD_RESPONSE section")
                    break
                elif subrecord_type == EGTS_SR_POS_DATA:
                    print("parsing EGTS_SR_POS_DATA section")
                    # NavigationTime
                    navigate_time = (
                    ...
                    flags = [
                        int(x)
                        for x in "{:08b}".format(
                            c_uint8(data[srd_offset + 12]).value
                        )
                    ]
                    if flags[2]:
                    ...
                    encoded_data_item = [
                        oid, date, time, lat_deg, long_deg, speed, course
                    ]
                    encoded_data.append(encoded_data_item)
                else:
                    print("unknown section type: ", subrecord_type)
